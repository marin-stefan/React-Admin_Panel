...
class App extends React.Component {
  ...
  displayComponent() {
    if (this.state.display === 'users') {
      return <UserList users={this.state.users} deleteUser={(id)=> this.deleteUserItem(id)}/>
    }else if (this.state.display === 'posts') {
      return <PostList posts={this.state.posts} deletePost={(id) => this.deletePostItem(id)} />
    }
  }
  ...
  render() {
    return(
      <div className="app" style={{background: this.state.background, color: this.state.color, display: this.state.display}}>
        <Header/>
        <NavBar backColorChange={(event) => this.changeColor(event)} 
